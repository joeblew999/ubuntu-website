---
title: "Yêu cầu đã nhận"
meta_title: "Cảm ơn | Ubuntu Software"
description: "Yêu cầu của bạn đã được nhận."
draft: false
aliases:
  - "/early-access/thank-you/"
---

## Thank You

Chúng tôi đã nhận được yêu cầu của quý khách và sẽ liên hệ lại với quý khách trong thời gian sớm nhất.

**Điều gì sẽ xảy ra tiếp theo:**

1. Chúng tôi sẽ xem xét yêu cầu của bạn trong vòng 2-3 ngày làm việc
2. Chúng tôi sẽ liên hệ để sắp xếp một cuộc gọi giới thiệu
3. Chúng tôi sẽ thảo luận về trường hợp sử dụng của bạn và giúp bạn bắt đầu

In the meantime, feel free to explore the [Platform](/platform/) or learn more about our [Founder](/company/founder/).

---

Questions? [Contact us](/contact/) anytime.
