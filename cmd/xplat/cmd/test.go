...
import (
	...
	"github.com/joeblew999/ubuntu-website/internal/taskfile"
...
const (
	// AffinityCross means tool can cross-compile from any platform
	AffinityCross Affinity = "cross"
	// AffinityNative means tool must build on native platform
...
func runTest(cmd *cobra.Command, args []string) error {
	...
	if arch.Type == taskfile.ArchetypeTool && affinity == AffinityNative {
...
// detectAffinity determines if a tool can cross-compile or needs native builds.
// This is runtime-agnostic - works for Go (CGO), Rust (native deps), etc.
func detectAffinity(tf *taskfile.Taskfile, arch taskfile.ArchetypeInfo) Affinity {
	// Only Tools have affinity concerns
	if arch.Type != taskfile.ArchetypeTool {
...
func printTestPlan(arch taskfile.ArchetypeInfo, namespace string, affinity Affinity) error {
